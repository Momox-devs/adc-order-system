...
class Orders extends BaseController
{
    ...
    public function getListProducts($paginate){
        $client = \Config\Services::curlrequest();
        $bodyRequest = json_decode($this->request->getBody());
        $options = [
            "headers" => [
        ...
        if($response->getStatusCode() == 200) {
            $responseBody    = json_decode($response->getBody());
            $items           = (array) $responseBody->items;
            $paginate        = (array) $responseBody->pagination;
            $currentPag      = $paginate['total'];
            $data['items']   = $items;
            return view('orders/products/All', $data);
        }else{
            return json_encode([
                "code"       =>  $response->getBody()->erroCode    ?: self::ERROR_UNKNOW, 
                "message"    =>  $response->getBody()->description ?: "Error al realizar la solicitud de productos"
    ...
    public function send(){
        ...
        if($response->getStatusCode() == 200) {
            ...
            $row_order = [
                "id_order"  => $order['id_order'],
                "status"    => "APROBADO",
                "return_id" => $responseBody->returnId,
            ];
