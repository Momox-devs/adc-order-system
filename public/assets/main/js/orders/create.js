    ...
    _getAllProducts : function(){
        ...
        windowHeight = 720;
         var left = (screen.width - windowWidth) / 2;
                    ...
                    var myWindow = window.open("/orders/products/all/1",'_blank','width=' + windowWidth + ', height=' + windowHeight + ', top=' + top + ', left=' + left);
    },
        ...
        $.each(thisO._products, function(index, product){
            if(!product['added'] && product['checked']){
                $(".table-selected-products").DataTable().row.add([
                    ...
                    product['barcode'],
                    product['description'],
                    "<input type='number' class='form-control input-sm input-width-100 cant-product-selected' value='"+(product['quantity'] ? product['quantity'] : 1)+"' id='cant-product-"+product['code']+"' />",
                ...
                thisO._products[index]['quantity_product'] = (product['quantity'] ? product['quantity'] : 1);
            }
        });
